...
def test_invalid_user_samples_wrong_dim():
    ...
    with pytest.raises(ValueError, match="user_samples must be a 3D numpy array"):
        multivariate_dame_bs_l_inf(np.array(user_scaled), alpha=0.5)
...
def test_values_out_of_range():
    ...
    with pytest.raises(ValueError, match="All entries must lie in"):
...
def test_invalid_n():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        result = multivariate_dame_bs_l_inf(np.array(user_samples), 0.6)
...
def test_about_smaller_m():
    ...
    """
    np.random.seed(42)
    user_samples,_ = generate_multivariate_scaled_data("normal", 10000, 2,2, [0.1,0.1])
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
